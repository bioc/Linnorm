...
\author{Shun H. Yip$^{1,2}$, Panwen Wang$^{1}$, Pak Chung Sham$^{1,3,4}$, Junwen Wang$^{1,5}$ \\[1em]
  ...
  \small{The University of Hong Kong, Hong Kong SAR, China.} \\
  \small{$^{5}$ Department of Health Sciences Research, Center for}  \\
  \small{Individualized Medicine, Mayo Clinic and Department of} \\
  \small{Biomedical Informatics, Arizona State University, Scottsdale,} \\
  \small{AZ 85259, USA.} \\
}
...
Many existing differential expression analysis methods assume the negative binomial or similar distributions for analysis\footnote{\Biocpkg{edgeR}, \Biocpkg{DESeq2}, \Biocpkg{DSS}, \Biocpkg{EBSeq}}. This approach performs well when the distribution in the RNA-seq dataset satisfies their assumptions. However, in practice, there are factors which can disturb the underlining distribution of the dataset. For example, a group of samples from one condition could come from distinct individuals instead of being replicates of each other; or, the dataset could come from a supervised learning algorithm that does not output data in the negative binomial distribution. To overcome this problem, the voom-\Biocpkg{limma} pipeline uses a different approach. Since \Biocpkg{limma} uses the linear models, which assumes homoscedasticity, voom models the dataset's mean-variance relationship and outputs a set of precision weights, which is a weight parameter for each gene, such that the empirical Bayes method in \Biocpkg{limma} can take homoscedasticity into consideration. However, there are issues that voom does not consider. First, voom utilizes logarithmic transformation. However, it does not consider the mean bias problem induced by performing logarithm on count datasets\footnote{O'hara, R. B. and Kotze, D. J. [2010]. Do not log-transform count data. Methods in Ecology and Evolution, 1 [2], 118-122.}. Second, other than homoscedasticity, voom does not seem to examine the other assumptions made by the linear models, such as normality in the residuals. Third, since voom gives a different weight parameter for each gene, it may be prone to over-fitting, especially when sample sizes are small. Therefore, Linnorm uses a different approach. Linnorm utilizes the mean bias induced by logarithmic transformation to transform the dataset toward homoscedasticity and normality; and it only has one transformation parameter for the whole dataset. This approach is less prone to over-fitting and the resulting dataset can better satisfy the assumptions made by the linear models, which is used by \Biocpkg{limma}. Furthermore, since Linnorm is actively transforming the dataset toward homoscedasticity and normality, it performs well with datasets which are deviated from the negative binomial distribution. Additionally, since homoscedasticity and normality are also assumed by many other parametric tests, such as the Pearson correlation coefficient, Linnorm transformed datasets are not restricted to differential expression analysis.
...
5. Linnorm-limma Transformation + Differential Expression Analysis
