...
* RNA-seq data simulation for negative binomial, Poisson, log normal or gamma distributions. \( RnaXSim \)
...
However, we discourage performing data imputation in most situations.
...
2. DEG Analysis
```{r, echo=TRUE}
#Example 1: Filter genes with more than 50% of the expressions being zero
#and genes with high technical noise.
scRNAseqResults <- Linnorm.limma(IslamData,design,minZeroPortion=0.5,Filter=TRUE)
...
tSNE.results <- Linnorm.tSNE(Islam2011[,1:92],
 Group=Groups, num_center=2, num_PC=2)
```
...
2. Simulate a scRNA-seq dataset.
```{r, echo=TRUE}
library(Linnorm)
...
2. Simulate a scRNA-seq dataset using the above parameters.
```{r, echo=TRUE}
library(Linnorm)
SimulatedData <- RnaXSim(SampleA,
...
5. During installation, an error says that Linnorm is not available. What is the problem?
Answer: You need to upgrade Bioconductor. You can upgrade Bioconductor by removing it with, "remove.packages("BiocInstaller")", then restart R session and install it again with, "source("http://bioconductor.org/biocLite.R")". Please also update R accordingly if needed.
