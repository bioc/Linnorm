...
Compared to RNA-seq or other large scale count data, scRNA-seq data have more noises and more zero counts. Linnorm is developed to work with scRNA-seq data. Of course, a good method that can cope with a high noise dataset can work with a dataset with fewer noise, such as RNA-seq data. Therefore, Linnorm is also compatible with RNA-seq data analysis.
...
By default, Linnorm's data imputation function replaces zerores in the dataset by inverse euclidean distance weighted mean, where weights are determined by PCA.
...
#We will not print all columns here
```
```{r,kable1, echo=FALSE}
library(knitr)
...
num_Clust=2, fontsize=1.5, Color = c("Red","Blue"))
```
...
1. Get 20 samples of TCGA RNA-seq data and convert it into TPM.
```{r, echo=TRUE}
library(Linnorm)
data(LIHC)
```
...
FCMatrix <- matrix(0, ncol=1, nrow=nrow(LIHC))
rownames(FCMatrix) <- rownames(LIHC)
colnames(FCMatrix) <- c("Log 2 Fold Change")
FCMatrix[,1] <- log((rowMeans(LIHC[,set1])+1)/(rowMeans(LIHC[,set2])+1),2)
...
We welcome any Bug Reports, Questions and Suggestions. They can be sent to Ken Yip at <shunyip@bu.edu> or posted on bioconductor's forum <https://support.bioconductor.org/>. We appreciate your help in making Linnorm better. Thanks!
