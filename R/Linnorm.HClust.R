...
#' @param method	Charcter. Method to be used in hierarchical clustering. (From hclust{stats}: the agglomeration method to be used. This should be (an unambiguous abbreviation of) one of "ward.D", "ward.D2", "single", "complete", "average" (= UPGMA), "mcquitty" (= WPGMA), "median" (= WPGMC) or "centroid" (= UPGMC).) Defaults to "ward.D2".
...
Linnorm.HClust <- function(datamatrix, showinfo = FALSE, input="Raw", perturbation=10, minZeroPortion=0, keepAll=TRUE, method="ward.D2", Group=NULL, num_Clust=4, ClustRect=TRUE, RectColor="red", fontsize=0.5, linethickness=0.5) {
